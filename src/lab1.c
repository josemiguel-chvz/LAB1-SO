...
int main(int argc, char **argv)
{
    ...
    int p = 0;
    int pipes[qty_disks*2][2];

    // Creación de Pipes y Procesos según cantidad de discos
    int i;
    for(i = 0; i < qty_disks; i++)
    {
        ...
        if(pid == 0)
        {
            ...
            close(pipes[p][READ]); // Cierra lectura pipe h->p
            close(pipes[p+1][WRITE]); // Cierra escritura pipe p->h
            fflush(stdout);

            int qty_visibilities = 0;
            char *visibilities[MAX_VISIBILITIES]; // Lista de argumentos para execve

            // Nombre de programa hijo
            char *child_program = "./vis";
            visibilities[0] = (char*)malloc(strlen(child_program)*sizeof(char));
            strcpy(visibilities[0], child_program);

            // String que recibe visibilidad desde padre
            char new_vis[200];

            // Leer desde pipe p->h mientras envie mensajes
            while (read(pipes[p+1][READ], &new_vis, sizeof(char) * 200) > 0) {
                // Si mensaje leido es fin, se termina procesamiento y se ejecuta programa hijo
                // De esta manera se finaliza el proceso hijo en el programa hijo
                if (strcmp(new_vis, "FIN") == 0) {
                    if (b_flag == 0)
                        printf("Fin del proceso hijo %d -> procesé %d visibilidades\n", getpid(), qty_visibilities);

                    // Pipe h -> padre direciona a contenido en STDOUT_FILENO
                    ...
                    // Se agregan visibilidades a lista de argumentos
    ...
    while(fgets(line, sizeof(line),file))
    {
        ...
        int pipe = disk + (disk+1); // Padre -> Hijo
        close(pipes[pipe-1][WRITE]); // Cierra escritura hijo-> padre
        close(pipes[pipe][READ]); // Cierra lectura padre->hijo
        // Escribir desde el padre al hijo un string de visibilidad
